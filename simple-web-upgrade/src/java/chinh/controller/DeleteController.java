...
public class DeleteController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            // cat khoang trang ten
            String checkName = lastName.trim();
            // xoa theo last name + userID
            dao.deleteByLastName(userID, checkName);
            // search len gia tri
            dao.searchLastName(searchValue);
            // get list account
            List<AccountDTO> result = dao.getListAccount();
            // set result len attribute
            request.setAttribute("SEARCHRESULT", result);
            // chuyen trang kem gia tri search
            request.getRequestDispatcher("search?txtSearchValue=" + searchValue).forward(request, response);
        }
    }
